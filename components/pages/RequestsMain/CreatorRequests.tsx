...
import { useGetCreatorRequests } from 'lib/api/queryHooks'
...
const CreatorRequests = () => {
  const { data, isFetching, refetch } = useGetCreatorRequests()
  ...
  const renderRequestCards = () => {
    switch (status) {
      case 2:
        return <Request2Cards requests={requests[2]} />
      case 3:
        return <Request3Cards requests={requests[3]} refetch={refetch} />
      case 4:
        return <Request4Cards requests={requests[4]} />
      case 5:
        return <Request5Cards requests={requests[5]} />
      default:
        return <Request1Cards requests={requests[1]} refetch={refetch} />
    }
  }
  ...
  return (
    <MainContainer>
      ...
      <button className={styles.reload} type="button" onClick={() => { refetch() }}>
        <Image fill sizes="100%" src="/icon-reload.png" alt="Reload" />
      </button>
      {renderRequestCards()}
    </MainContainer>
  )
